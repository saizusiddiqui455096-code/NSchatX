/* NS ChatX client — auth via REST, live messages over WebSocket. */
(() => {
  'use strict';

  const PORT_TOKEN = 'port/8000';
  const isLocalHost =
    !location.hostname || location.hostname === 'localhost' || location.hostname === '127.0.0.1';
  const isLocal = isLocalHost || PORT_TOKEN.startsWith('__');
  const API = isLocal ? 'http://localhost:8000' : PORT_TOKEN;

  function wsURL(token) {
    if (isLocal) return `ws://${location.hostname || 'localhost'}:8000/ws?token=${token}`;
    const base = location.pathname.replace(/\/[^/]*$/, '');
    const proto = location.protocol === 'https:' ? 'wss:' : 'ws:';
    return `${proto}//${location.host}${base}/${PORT_TOKEN}/ws?token=${token}`;
  }
  ...
  function setMode(next) {
    ...
    el.roomId.placeholder = creating ? 'e.g. TEAM-42 (or generate)' : 'e.g. TEAM-42';
